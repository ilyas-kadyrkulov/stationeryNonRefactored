...
export default createStore({
  ...
  actions: {
    async getPizzas({ commit }) {
      const response = await fetch("https://stationery-vue-backend-pitb.vercel.app/stationery");
    ...
    async setCategory({ commit, state }, value) {
      ...
      const response = await fetch(
        `https://stationery-vue-backend-pitb.vercel.app/stationery${query}&_order=desc`
      );
      const json = await response.json();
    ...
    async fetchSearchResults({ commit }, searchValue) {
      const response = await fetch(
        `https://stationery-vue-backend-pitb.vercel.app/stationery/?q=${searchValue}`
      );
